...
def _init_cred(account_name):
    cfg_path = f"cloud.aws.s3.{account_name}"
    gcfg = cfg.cd(cfg_path)
    creds_account_name = gcfg.setdefault("credentials", account_name)
    _cred_registry[account_name] = plpipes.cloud.aws.auth.credentials(creds_account_name)
...
def s3(account_name):
    if account_name not in _s3_registry:
        _init_s3(account_name)
    return _s3_registry[account_name]

def _init_s3(account_name):
    credential=_cred(account_name)
    _s3_registry[account_name] = s3
